...
export default function TournamentDetail() {
      ...
      .then((d) => {
        setData(
          d.success
            ? d
            : {
                tournament: d.data ?? d.tournament,
                teams: d.teams ?? [],
                leaderboard: d.leaderboard ?? [],
                totalTeams: d.totalTeams ?? 0,
              },
        );
        setTimeout(() => setBarFill(true), 600);
      })
      .catch(() => {})
      .finally(() => {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    setTimeLeft(countdown(data.tournament.start_date)); // set immediately, no delay on first render
    const iv = setInterval(
      () => setTimeLeft(countdown(data.tournament.start_date)),
      1000,
    );
  ...
  if (loading) {
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <FontAwesomeIcon
          icon={faSpinner}
          className="animate-spin text-[#F2AA00] w-8 h-8"
        />
      </div>
    );
  }
    ...
    return (
      <div className="bg-black min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <button
          ...
          className="text-[#F2AA00] text-xs tracking-widest hover:underline"
        >
          ← Back to Tournaments
        </button>
      </div>
  ...
  const infoCards = [
    ...
    { icon: faCalendar, label: "Date", val: t.startFormatted + " IST" }, // ✅ API already formats this with date and time
  ...
  return (
    <div className="bg-black min-h-screen text-white">
      {/* ── HERO BANNER ── */}
      <div
        className={`relative h-56 sm:h-72 bg-gradient-to-b ${mapGrad} overflow-hidden`}
      >
        ...
        {/* TITLE */}
        <div
          className={`absolute bottom-6 left-6 right-6 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          <div className="flex items-center gap-3 mb-2">
            <span
              className={`text-[11px] px-2.5 py-1 rounded-md border tracking-widest ${statusStyle[t.status] ?? "border-gray-700 text-gray-500"}`}
            >
              {t.status}
            </span>
          ...
          {t.description && (
            <p className="text-gray-400 text-sm mt-1 tracking-wide max-w-lg">
              {t.description}
            </p>
          )}
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        {/* COUNTDOWN + JOIN */}
        <div
          className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          <div>
            <p className="text-[10px] text-gray-600 tracking-widest uppercase">
              Tournament Starts In
            </p>
            <p className="text-[#F2AA00] font-mono text-2xl mt-1 tracking-wide">
              {timeLeft}
            </p>
          </div>
          <button
            disabled={!canJoin}
            onClick={() =>
              canJoin && router.push(`/tournaments/${id}/register`)
            }
        ...
        {/* SLOT PROGRESS */}
        <div
          className={`bg-[#0b0b0b] border border-gray-800 rounded-xl px-5 py-4 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        ...
        >
          {infoCards.map((card, i) => (
            ...
            >
              ...
              <p className="text-sm text-white tracking-wide truncate">
                {card.val}
              </p>
            </div>
          ))}
        ...
        <div
          className={`grid md:grid-cols-2 gap-5 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        ...
        >
          ...
          <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl overflow-hidden">
            ...
            {leaderboard.length === 0 ? (
              <p className="text-center py-10 text-gray-700 text-xs tracking-widest">
                No results yet — tournament in progress.
              </p>
            ) : (
              <div className="divide-y divide-gray-800/50">
                {leaderboard.map((row: any, i: number) => (
                  <div
                    key={i}
                    className="flex items-center justify-between px-5 py-3.5 hover:bg-[#111] transition-colors duration-150"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-lg w-8 text-center">
                        {positionEmoji[i + 1] ?? `#${i + 1}`}
                      </span>
                    ...
                    <span
                      className={`font-mono text-sm ${positionColor[i + 1] ?? "text-gray-500"}`}
                    >
                      +{row.points} pts
                    </span>
                  </div>
          ...
          <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl overflow-hidden">
            ...
            >
              ...
              <FontAwesomeIcon
                icon={rulesOpen ? faChevronUp : faChevronDown}
                className="text-gray-500 text-xs"
              />
            </button>
            <div
              className={`overflow-hidden transition-all duration-300 ${rulesOpen ? "max-h-96" : "max-h-32"}`}
            >
              <div className="px-5 py-4">
                {t.rules ? (
                  <p className="text-gray-400 text-sm leading-relaxed tracking-wide whitespace-pre-line">
                    {t.rules}
                  </p>
                ) : (
                  <ul className="space-y-2 text-gray-400 text-sm">
                    {[
                      ...
                      <li key={i} className="flex items-start gap-2">
                        <FontAwesomeIcon
                          icon={faCircle}
                          className="text-[#F2AA00] text-[5px] mt-1.5 flex-shrink-0"
                        />
                        <span>{r}</span>
                      </li>
                    ))}
            ...
            {!rulesOpen && (
              <div className="px-5 pb-3">
                <button
                  ...
                  className="text-[10px] text-[#F2AA00]/70 hover:text-[#F2AA00] tracking-widest transition-colors"
                >
                  Show more →
                </button>
              </div>
        ...
        >
          ...
          >
            ...
            <FontAwesomeIcon
              icon={teamsOpen ? faChevronUp : faChevronDown}
              className="text-gray-500 text-xs"
            />
          </button>
              ...
              {teams.length === 0 ? (
                ...
                teams.map((team: any, i: number) => (
                  <div
                    ...
                    className="px-5 py-4 hover:bg-[#111] transition-colors duration-150"
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {team.teamTag && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-500 tracking-widest">
                                [{team.teamTag}]
                              </span>
                            )}
                          </div>
                          <p className="text-[11px] text-gray-500 mt-0.5">
                            <FontAwesomeIcon
                              icon={faCrown}
                              className="text-[#F2AA00] text-[9px] mr-1"
                            />
                            {team.captainName}
                          </p>
                        </div>
        ...
        {canJoin && (
          <div
            className={`bg-[#F2AA00]/5 border border-[#F2AA00]/20 rounded-xl px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          ...
          >
            <div>
              ...
              <p className="text-gray-500 text-xs mt-0.5 tracking-wide">
                {t.slots - t.filled} slots left · Entry {t.fee} · Prize{" "}
                {t.prize}
              </p>
            </div>
            <button
