...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-4">
      ...
      <p className="text-[13px] text-gray-500 tracking-[0.2em] uppercase">
        {children}
      </p>
    </div>
  );
...
function PlayerSlot({ index, value, onChange, isCaptain = false }: any) {
  return (
    <div
      className={`p-4 rounded-xl border transition-all duration-200 ${isCaptain ? "border-[#F2AA00]/30 bg-[#F2AA00]/5" : "border-gray-800 bg-black/30 hover:border-gray-700"}`}
    >
      <div className="flex items-center gap-2 mb-3">
        <div
          className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] flex-shrink-0 ${isCaptain ? "bg-[#F2AA00] text-black" : "bg-gray-800 text-gray-400"}`}
        >
          {isCaptain ? "C" : index}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="relative">
          <FontAwesomeIcon
            icon={faUser}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 text-xs pointer-events-none"
          />
          <input
            placeholder="In-game Name"
            value={value.name}
            onChange={(e) => onChange("name", e.target.value)}
            className={inputCls + " pl-8 text-[14px]"}
          />
        </div>
        <div className="relative">
          <FontAwesomeIcon
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      router.replace(
        `/login?message=Please login to continue&redirect=/tournaments/${id}/register`,
      );
    } else {
      setAuthChecked(true);
    }
    ...
    fetch(`/api/tournaments/${id}`, { cache: "no-store" })
      ...
      .then((data) => {
        ...
        setTournament({
          ...
          playerCount:
            raw.mode?.toLowerCase() === "solo"
              ? 1
              : raw.mode?.toLowerCase() === "duo"
                ? 2
                : 4,
  ...
  useEffect(() => {
    ...
    setPlayers(
      Array.from({ length: tournament.playerCount - 1 }, () => ({
        name: "",
        id: "",
      })),
    );
  ...
  const updatePlayer = (idx: number, field: "name" | "id", val: string) =>
    setPlayers((p) =>
      p.map((pl, i) => (i === idx ? { ...pl, [field]: val } : pl)),
    );
  ...
  const slotPercent = tournament
    ? Math.round((tournament.filled / tournament.slots) * 100)
    : 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!token) {
        router.replace(
          `/login?message=Session expired. Please login again.&redirect=/tournaments/${id}/register`,
        );
        return;
      }
      ...
      <div className="bg-black text-white min-h-screen flex items-center justify-center px-4">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-gray-500 text-md leading-relaxed">
            Your payment is under review. Admin will verify and confirm your
            slot for <span className="text-[#F2AA00]">{tournament?.name}</span>.
          </p>
  ...
  return (
    <div className="bg-black text-white min-h-screen px-4 py-10 md:px-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        {/* HEADER */}
        <div
          className={`mb-8 transition-all duration-600 ${visible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}
        >
          <p className="text-[10px] tracking-[0.4em] text-[#F2AA00] uppercase mb-2">
            Registration
          </p>
          <h1 className="text-3xl sm:text-4xl tracking-wide">
          ...
          >
            ...
            {tournament.playerCount > 1 && (
              <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl p-5">
                ...
                <div className="relative tracking-widest">
                  <FontAwesomeIcon
                    icon={faShield}
                    className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-600 text-xs pointer-events-none"
                  />
                  <input
            ...
            <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl p-5">
              ...
              <div className="flex items-center justify-between bg-black border border-gray-800 rounded-xl px-4 py-3.5 mb-4">
                <div>
                  <p className="text-[10px] text-gray-600 tracking-widest uppercase mb-0.5">
                    Entry Fee
                  </p>
                  <p className="text-sm text-gray-500">{tournament.name}</p>
                </div>
                <p className="text-2xl text-[#F2AA00] font-mono">
                  {tournament.fee}
                </p>
              </div>
              ...
              <div className="flex items-start gap-2.5 text-[14px] text-gray-500 mb-4 bg-[#F2AA00]/5 border border-[#F2AA00]/10 rounded-lg px-3 py-2.5">
                <FontAwesomeIcon
                  ...
                  className="text-[#F2AA00]/60 flex-shrink-0 mt-0.5"
                />
                <span>
                  Pay {tournament.fee} via UPI, upload your payment screenshot
                  and enter the transaction ID. Admin will verify and approve
                  your slot.
                </span>
              </div>
              ...
              <div className="relative mb-3">
                <FontAwesomeIcon
                  icon={faMobileScreen}
                  className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-600 text-md pointer-events-none"
                />
                <input
                  placeholder="Your UPI ID (e.g. name@upi)"
                  value={upi}
                  onChange={(e) => setUpi(e.target.value)}
                  className={inputCls + " pl-9 text-[14px]"}
                />
              </div>
              ...
              <label className="block mb-3 cursor-pointer">
                <div
                  className={`border-2 border-dashed rounded-xl p-5 text-center transition-all duration-300 ${screenshot ? "border-green-500/40 bg-green-500/5" : "border-gray-800 hover:border-[#F2AA00]/30 hover:bg-[#F2AA00]/3"}`}
                >
                  {screenshot ? (
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={inputCls + " pl-9 font-mono text-[14px]"}
                />
              </div>
            </div>
            ...
            <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition-colors duration-200">
              <div className="relative h-32 overflow-hidden">
                ...
                <div className="absolute bottom-3 left-4">
                  ...
                  <p className="text-white text-sm tracking-wide mt-0.5">
                    {tournament.name}
                  </p>
                </div>
              ...
              <div className="p-4 space-y-3">
                {[
                  {
                    label: "Mode",
                    val: `${tournament.mode} · ${tournament.playerCount} Players`,
                  },
                  {
                    label: "Date",
                    val: `${tournament.date} · ${tournament.time}`,
                  },
                ...
                <div>
                  <div className="flex justify-between text-[10px] text-gray-600 mb-1.5">
                    <span>Slots</span>
                    <span>
                      {tournament.filled}/{tournament.slots} · {slotsLeft} left
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {ALL_MAPS.map((map) => {
                  ...
                  return (
                    <div
                      key={map.name}
                      className={`relative rounded-lg overflow-hidden border transition-all duration-300 ${isSelected ? "border-[#F2AA00] shadow-md shadow-[#F2AA00]/20" : "border-gray-800 opacity-50"}`}
                    >
                      <Image
                        src={map.src}
                        alt={map.name}
                      ...
                      />
                      <div
                        className={`absolute inset-0 transition-all duration-300 ${isSelected ? "bg-black/20" : "bg-black/50"}`}
                      />
