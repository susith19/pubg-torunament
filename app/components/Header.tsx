          ...
          {(item as any).dropdown.map((d: any, i: number) => (
            <a
              ...
              className={`block px-5 py-3 text-[11px] tracking-[0.12em] text-gray-300 hover:text-[#F2AA00] hover:bg-white/5 transition-colors duration-150 ${
                i !== (item as any).dropdown.length - 1
        ...
        <div className="bg-gray-900 rounded-xl overflow-hidden shadow-2xl">
          ...
          <div className="px-4 py-3 border-b border-white/10 flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              <p className="text-white text-[12px] tracking-[0.10em] font-medium truncate">
                {user.name}
              </p>
              {user.email && (
                <p className="text-gray-500 text-[10px] tracking-[0.08em] truncate">
                  {user.email}
                </p>
              )}
            </div>
...
export default function Header() {
  ...
  return (
    <header
      className="sticky top-0 z-50 bg-[#F2AA00] flex items-center justify-between px-8 py-0 relative shadow-md"
      style={{ minHeight: 56 }}
    >
      {/* LEFT — Logo */}
      ...
      >
        <span
          className={`block w-5 h-px bg-black transition-all duration-300 ${mobileOpen ? "rotate-45 translate-y-[7px]" : ""}`}
        />
        <span
          className={`block w-5 h-px bg-black transition-all duration-300 ${mobileOpen ? "opacity-0" : ""}`}
        />
        <span
          className={`block w-5 h-px bg-black transition-all duration-300 ${mobileOpen ? "-rotate-45 -translate-y-[7px]" : ""}`}
        />
