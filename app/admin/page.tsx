...
// ── icon map (API sends string keys, we map to FA icons) ──
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <p className="text-gray-500 text-sm tracking-widestst uppercase">{title}</p>
        ...
        <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#F2AA00]/10 group-hover:bg-[#F2AA00]/20 transition-colors duration-300">
          <FontAwesomeIcon icon={iconMap[iconKey] ?? faTrophy} className="text-[#F2AA00] text-md" />
        </div>
      </div>
...
export default function AdminDashboard() {
  ...
  if (error || !data) {
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <p className="text-red-400 text-md">Failed to load dashboard: {error}</p>
      </div>
    );
  }
    ...
    <div className="bg-black min-h-screen text-white px-4 sm:px-6 py-10">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className={`transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-3"}`}>
          <h1 className="text-xl tracking-widestst text-white">Admin Dashboard</h1>
          <p className="text-gray-600 text-sm mt-1 tracking-widest">Overview of platform activity</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div>
                <p className="text-md tracking-widest">Weekly Signups</p>
                <p className="text-gray-500 text-sm mt-0.5">Users registered this week</p>
              </div>
              <span className="text-[10px] border border-gray-800 text-gray-500 px-2.5 py-1 rounded-full tracking-widestst">
                This Week
              </span>
            </div>
          ...
          >
            <p className="text-md tracking-widest mb-4">Recent Activity</p>
            {activity.length === 0 ? (
              <p className="text-gray-700 text-sm tracking-widestst text-center py-6">No activity yet.</p>
            ) : (
                ...
                {activity.map((a: any, i: number) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="text-md text-gray-300 leading-snug tracking-widestst">{a.text}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          >
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-800">
              <p className="text-md tracking-widest">Recent Tournaments</p>
              <a href="/admin/tournaments" className="text-[10px] text-[#F2AA00]/70 hover:text-[#F2AA00] transition-colors tracking-widestst">View All →</a>
            </div>
            {recentTournaments.length === 0 ? (
              <p className="text-gray-700 text-sm tracking-widestst text-center py-8">No tournaments yet.</p>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-800/60">
                    {["Name", "Mode", "Slots", "Status", "Fee"].map((h) => (
                      <th key={h} className="text-[10px] text-gray-600 tracking-widestst uppercase px-4 py-2.5 text-left">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {recentTournaments.map((t: any, i: number) => (
                    <tr key={i} className={`border-b border-gray-800/40 last:border-0 hover:bg-[#111] transition-colors duration-150 ${visible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: `${560 + i * 60}ms` }}>
                      <td className="px-4 py-3">
                        <p className="text-md text-white tracking-widestst">{t.name}</p>
                        <p className="text-[10px] text-gray-600 mt-0.5">{t.map}</p>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-500">{t.mode}</td>
                      <td className="px-4 py-3 text-sm text-gray-500 font-mono">{t.slots}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] px-2 py-0.5 rounded-md border tracking-widest ${statusColor[t.status] ?? "border-gray-700 text-gray-500"}`}>{t.status}</span>
                      </td>
                      <td className="px-4 py-3 text-sm text-[#F2AA00]/80">{t.fee}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          >
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-800">
              <p className="text-md tracking-widest">Recent Users</p>
              <a href="/admin/users" className="text-[10px] text-[#F2AA00]/70 hover:text-[#F2AA00] transition-colors tracking-widestst">View All →</a>
            </div>
            {recentUsers.length === 0 ? (
              <p className="text-gray-700 text-sm tracking-widestst text-center py-8">No users yet.</p>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-800/60">
                ...
                <tbody>
                  {recentUsers.map((u: any, i: number) => (
                    <tr key={i} className={`border-b border-gray-800/40 last:border-0 hover:bg-[#111] transition-colors duration-150 ${visible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: `${600 + i * 60}ms` }}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-md text-white tracking-widestst">{u.name}</p>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-[10px] px-2 py-0.5 rounded-md border tracking-widest ${statusColor[u.status] ?? "border-gray-700 text-gray-500"}`}>{u.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {/* QUICK ACTIONS */}
        <div
          className={`grid grid-cols-2 sm:grid-cols-4 gap-3 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          style={{ transitionDelay: "680ms" }}
        >
          {[
            { label: "Create Tournament", gold: true,  href: "/admin/tournaments" },
            { label: "Manage Users",      gold: false, href: "/admin/users"       },
            { label: "Process Redeems",   gold: false, href: "/admin/redeems"     },
            { label: "Verify Payments",   gold: false, href: "/admin/payments"    },
          ].map((btn, i) => (
            <a
              key={i}
              href={btn.href}
              className={`py-2.5 text-sm tracking-widestst rounded-lg border text-center transition-all duration-150 active:scale-[0.97] ${btn.gold ? "bg-[#F2AA00] text-black border-[#F2AA00] hover:bg-[#e09e00] hover:shadow-lg hover:shadow-[#F2AA00]/20" : "bg-transparent text-gray-400 border-gray-800 hover:border-gray-700 hover:text-white"}`}
            >
              {btn.label}
            </a>
          ))}
