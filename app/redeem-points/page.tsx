...
import {
  faCoins, faTrophy, faCircleCheck, faClock, faXmark,
  faCopy, faCheck, faIndianRupeeSign, faMobileScreen,
  faBuildingColumns, faLock, faGift, faHeadset, faPlus,
  faArrowRight,
...
const INITIAL_POINTS = {
  total:    420,
  referral: 120,   // auto-calculated from referral uses
  winning:  300,   // added by admin when team wins
  redeemed: 0,
};
...
const REFERRAL_CONFIG = {
  code: "SUSITH123",
  minPts: 1,
  maxPts: 100,     // random per referral — matches sketch "1 to 100"
};

const WINNING_HISTORY = [
  { id: 1, tournament: "Erangel Squad Battle", position: 1, points: 150, date: "Nov 28, 2026", badge: "gold"   },
  { id: 2, tournament: "Miramar Duo Clash",    position: 2, points: 80,  date: "Nov 20, 2026", badge: "silver" },
  { id: 3, tournament: "Sanhok Solo Rush",     position: 3, points: 40,  date: "Nov 15, 2026", badge: "bronze" },
  { id: 4, tournament: "TDM Arena",            position: 1, points: 30,  date: "Nov 10, 2026", badge: "gold"   },
];

const REFERRAL_HISTORY_INIT = [
  { id: 1, user: "Arjun",   earned: 45, date: "2 days ago" },
  { id: 2, user: "Meena",   earned: 72, date: "5 days ago" },
  { id: 3, user: "Karthik", earned: 3,  date: "1 week ago" },
];

const REDEEM_HISTORY_INIT = [
  { id: 1, amount: 200, method: "UPI",  detail: "susith@upi",    status: "Approved", date: "Nov 1, 2026"  },
  { id: 2, amount: 150, method: "Bank", detail: "HDFC ****4321", status: "Pending",  date: "Nov 25, 2026" },
];
...
function RedeemModal({ available, onClose, onSubmit }: {
  available: number;
  onClose: () => void;
  onSubmit: (pts: number, method: string, detail: string) => void;
}) {
  const MIN = 200;
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <button
                onClick={() => setPts(available)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[#F2AA00] hover:text-[#F2AA00]/70 tracking-widest transition-colors"
              >
                MAX
              </button>
            </div>
            <div className="mt-2 h-1.5 bg-gray-900 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-400 ${over || under ? "bg-red-500" : "bg-[#F2AA00]"}`}
                style={{ width: `${barW}%` }}
              />
            </div>
          ...
          <button
            onClick={() => valid && onSubmit(pts, method, detail)}
            className={`w-full py-3 text-xs tracking-widest rounded-lg border transition-all duration-150 active:scale-[0.97] ${valid ? "bg-[#F2AA00] text-black border-[#F2AA00] hover:bg-[#e09e00] hover:shadow-lg hover:shadow-[#F2AA00]/20" : "bg-gray-900 text-gray-600 border-gray-800 cursor-not-allowed"}`}
          >
            {valid ? "Confirm Redeem" : under ? `Need ${MIN - pts} more pts` : over ? "Exceeds balance" : "Enter payout details"}
          </button>

          {/* customer service */}
          <a
            href="/contact"
            className="flex items-center justify-center gap-2 text-[10px] text-gray-600 hover:text-[#F2AA00] transition-colors duration-200 tracking-wide mt-1"
          >
...
export default function PointsPage() {
  const [pts, setPts]                   = useState(INITIAL_POINTS);
  const [refHistory, setRefHistory]     = useState(REFERRAL_HISTORY_INIT);
  const [redeemHist, setRedeemHist]     = useState(REDEEM_HISTORY_INIT);
  ...
  // copy referral code → simulate someone using it after 3s
  const handleCopy = () => {
    navigator.clipboard?.writeText(REFERRAL_CONFIG.code).catch(() => {});
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
    pop("Referral code copied!");

    // AUTO-CALCULATE referral points: random 1–100 added when code is used
    setTimeout(() => {
      const earned = Math.floor(
        Math.random() * (REFERRAL_CONFIG.maxPts - REFERRAL_CONFIG.minPts + 1)
      ) + REFERRAL_CONFIG.minPts;
      setPts((p) => ({ ...p, total: p.total + earned, referral: p.referral + earned }));
      setRefHistory((prev) => [
        { id: Date.now(), user: "New User", earned, date: "Just now" },
        ...prev,
      ]);
      pop(`+${earned} referral points auto-added!`);
    }, 3000);
  };

  const handleRedeem = (amount: number, method: string, detail: string) => {
    setPts((p) => ({ ...p, total: p.total - amount, redeemed: p.redeemed + amount }));
    setRedeemHist((prev) => [
      { id: Date.now(), amount, method, detail, status: "Pending", date: "Just now" },
      ...prev,
    ]);
    setShowRedeem(false);
  ...
  const totalRef  = refHistory.reduce((a, r) => a + r.earned, 0);
  ...
  return (
    <div className="bg-black min-h-screen text-white px-4 sm:px-6 py-10 relative overflow-x-hidden">
      ...
      {showRedeem && (
        <RedeemModal available={available} onClose={() => setShowRedeem(false)} onSubmit={handleRedeem} />
      )}
      ...
      <div className="max-w-4xl mx-auto space-y-5 relative z-10">
        ...
        <div className={`flex items-start justify-between transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
          ...
          {/* customer service link — from sketch */}
          <a
            href="/contact"
            className="hidden sm:flex items-center gap-2 text-[10px] text-gray-600 hover:text-[#F2AA00] border border-gray-800 hover:border-[#F2AA00]/30 px-3 py-2 rounded-lg transition-all duration-200 tracking-wide flex-shrink-0 mt-1"
          >
            <FontAwesomeIcon icon={faHeadset} className="text-[9px]" />
            Customer Service
          </a>
        ...
        {/* ── BREAKDOWN — winning + referral ── */}
        <div
          className={`grid sm:grid-cols-2 gap-4 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          style={{ transitionDelay: "160ms" }}
        >
          {/* WINNING POINTS — added by admin per team result */}
          <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl p-5 group hover:border-[#F2AA00]/30 hover:-translate-y-0.5 transition-all duration-300">
            ...
            <div className="h-1.5 bg-gray-900 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: visible ? `${(pts.winning / (pts.total || 1)) * 100}%` : "0%" }}
              />
            </div>
            <p className="text-[9px] text-gray-700 mt-1.5 tracking-wide">
              {Math.round((pts.winning / (pts.total || 1)) * 100)}% of total
            </p>
          </div>

          {/* REFERRAL POINTS — auto-calculated */}
          <div className="bg-[#0b0b0b] border border-gray-800 rounded-xl p-5 group hover:border-[#F2AA00]/30 hover:-translate-y-0.5 transition-all duration-300">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-[#F2AA00]/10 flex items-center justify-center group-hover:bg-[#F2AA00]/20 transition-colors duration-200">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-gray-600 mt-0.5">Earn {REFERRAL_CONFIG.minPts}–{REFERRAL_CONFIG.maxPts} pts per referral</p>
                </div>
              </div>
            ...
            <div className="h-1.5 bg-gray-900 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#F2AA00]/70 rounded-full transition-all duration-700"
                style={{ width: visible ? `${(pts.referral / (pts.total || 1)) * 100}%` : "0%" }}
              />
            </div>
            <p className="text-[9px] text-gray-700 mt-1.5 tracking-wide">
              {Math.round((pts.referral / (pts.total || 1)) * 100)}% of total · {refHistory.length} referrals
            </p>
          </div>
        </div>
        ...
        {/* ── REFERRAL CODE ── */}
        <div
          className={`bg-[#0b0b0b] border border-gray-800 rounded-xl p-5 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          style={{ transitionDelay: "240ms" }}
        >
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-,d text-white tracking-wide">Refer to Earn Points</p>
              <p className="text-[10px] text-gray-600 mt-0.5 tracking-wide">
                Share your code — earn <span className="text-[#F2AA00]">{REFERRAL_CONFIG.minPts} to {REFERRAL_CONFIG.maxPts} points</span> per referral, auto-added
              </p>
            </div>
            <span className="text-[10px] text-gray-600 border border-gray-800 px-2.5 py-1 rounded-full tracking-widest flex-shrink-0 ml-3">
              {refHistory.length} used
            </span>
          </div>
          ...
          <div className="flex items-center justify-between bg-black border border-gray-800 hover:border-[#F2AA00]/30 rounded-xl px-4 py-3.5 mb-3 transition-colors duration-200">
            <p className="text-[#F2AA00] font-mono tracking-[0.35em] text-sm">{REFERRAL_CONFIG.code}</p>
            <button
              onClick={handleCopy}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all duration-200 ${copied ? "border-[#F2AA00]/50 bg-[#F2AA00]/10 text-[#F2AA00]" : "border-gray-700 text-gray-400 hover:border-gray-600 hover:text-white"}`}
            >
            ...
            {refHistory.slice(0, 3).map((r) => (
              <div key={r.id} className="flex items-center justify-between px-3 py-2 bg-black/40 rounded-lg border border-gray-800/50 hover:border-gray-800 transition-colors">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-[#F2AA00]/20 flex items-center justify-center text-[8px] text-[#F2AA00]">
                    {r.user[0]}
                  </div>
                  <p className="text-[14px] text-gray-500">{r.user} used your code</p>
                </div>
        ...
        {/* ── HISTORY TABS ── */}
        <div
          className={`bg-[#0b0b0b] border border-gray-800 rounded-xl overflow-hidden transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        ...
        >
          ...
          {tab === "winning" && (
            <div className="p-4 space-y-2">
              {WINNING_HISTORY.map((w, i) => (
                <div
                  key={w.id}
                  className={`flex items-center justify-between px-4 py-3.5 rounded-xl border transition-all duration-300 hover:-translate-x-0.5 ${badgeStyle[w.badge]}`}
                  style={{ transitionDelay: `${i * 50}ms` }}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xl flex-shrink-0">{badgeEmoji[w.badge]}</span>
                    <div>
                      <p className="text-xs text-white tracking-wide">{w.tournament}</p>
                      <p className="text-[10px] text-gray-500 mt-0.5">{posLabel[w.position]} Place · {w.date}</p>
                    </div>
                  </div>
            ...
            <div className="p-4 space-y-2">
              {refHistory.length === 0 ? (
                ...
                <div key={r.id} className="flex items-center justify-between px-4 py-3 bg-black border border-gray-800 rounded-xl hover:border-gray-700 transition-colors duration-150">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#F2AA00]/20 flex items-center justify-center text-xs text-[#F2AA00]">{r.user[0]}</div>
                    <div>
            ...
            <div className="p-4 space-y-2">
              {redeemHist.length === 0 ? (
                ...
                <div key={r.id} className="flex items-center justify-between px-4 py-3.5 bg-black border border-gray-800 rounded-xl hover:border-gray-700 transition-colors duration-150">
                  ...
                  <span className={`text-[10px] px-2.5 py-1 rounded-md border tracking-wide flex-shrink-0 ${statusStyle[r.status]}`}>
                    {r.status}
                  </span>
                </div>
              ))}
        ...
        {/* ── FOOTER: min redeem info + customer service ── */}
        <div
          className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#0b0b0b] border border-gray-800 rounded-xl px-5 py-4 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
